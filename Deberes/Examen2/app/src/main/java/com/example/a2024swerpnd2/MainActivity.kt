...
class MainActivity : AppCompatActivity() {
    ...
    override fun onContextItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.editar_cliente_menu -> {
                val clienteSeleccionado: Cliente? =
                    obtenerClienteSeleccionado(posicionitemSeleccionadoMenu)
            ...
            R.id.agregar_ubicacion -> {
                val clienteSeleccionado: Cliente? =
                    obtenerClienteSeleccionado(posicionitemSeleccionadoMenu)
                if (clienteSeleccionado!!.latitud != 0.0 || clienteSeleccionado!!.longitud != 0.0) {
                    mostrarSnackbar("El cliente ${clienteSeleccionado.nombre} ya tiene ubicación.")
                } else {
                    empezarMapsActivity(clienteSeleccionado)
                }
                return true
            }
            ...
            R.id.ver_ubicacion -> {
                val clienteSeleccionado: Cliente? =
                    obtenerClienteSeleccionado(posicionitemSeleccionadoMenu)
                if (clienteSeleccionado?.latitud?.isNaN() == true || clienteSeleccionado?.longitud == 0.0) {
                    mostrarSnackbar("Primero agrega una ubicación para el cliente ${clienteSeleccionado.nombre}.")
                } else {
                    empezarMapsActivity(clienteSeleccionado)
                }
                return true
    ...
    private fun empezarMapsActivity(clienteSeleccionado: Cliente?) {
        val intentMaps = Intent(this, GoogleMapsActivity::class.java)
        intentMaps.putExtra("idCliente", clienteSeleccionado!!.idCliente)
        intentMaps.putExtra("latitud", clienteSeleccionado!!.latitud)
        intentMaps.putExtra("longitud", clienteSeleccionado!!.longitud)
