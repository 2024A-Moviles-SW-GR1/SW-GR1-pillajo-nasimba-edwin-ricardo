...
class SQLiteHelper(contexto: Context):
        ...
        val scriptSQLCrearTablaCliente =
            """
                CREATE TABLE CLIENTE(
                    ...
                    estadoCivil CHAR(1),
                    edad INTEGER,
                    latitud DECIMAL(9, 6) DEFAULT 0.0,  -- Nuevos atributos
                    longitud DECIMAL(9, 6) DEFAULT 0.0 -- Nuevos atributos
                )
            """.trimIndent()
    ...
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        if (oldVersion < 2) {
            val scriptSQLColumnalatActualizarCliente =
                """
                ALTER TABLE CLIENTE
                ADD COLUMN latitud DOUBLE
            """.trimIndent()
            val scriptSQLColumnaLongActualizarCliente =
                """
                ALTER TABLE CLIENTE
                ADD COLUMN longitud DOUBLE
            """.trimIndent()
            db?.execSQL(scriptSQLColumnalatActualizarCliente)
            db?.execSQL(scriptSQLColumnaLongActualizarCliente)
        }
    }
        ...
        if(resultadoConsultaLectura.moveToFirst()){
            do {
                val cliente = Cliente(resultadoConsultaLectura.getInt(0),
                    ...
                    resultadoConsultaLectura.getDouble(6), // Leer latitud
                    resultadoConsultaLectura.getDouble(7)  // Leer longitud
                )
    ...
    fun actualizarUbicacionCliente(latitude: Double, longitude: Double, idCliente: Int?): Boolean {
        if (idCliente == null) {
            return false // No se puede actualizar si idCliente es null
        }
        val conexionEscritura = writableDatabase
